...
int main() {
    ...
    commandPool0.AllocateBuffers(commandBuffer[0]);
    commandPool1.AllocateBuffers(commandBuffer[1]);
    ...
    auto BeforeSuspending = [&, imageMemoryBarrier, renderingInfo]() mutable {
        commandBuffer[0].Begin(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
        ...
        vkCmdBeginRendering(commandBuffer[0], &renderingInfo);
        vkCmdEndRendering(commandBuffer[0]);

        commandBuffer[0].End();
    };
    auto AfterResuming = [&, imageMemoryBarrier, renderingInfo]() mutable {
        commandBuffer[1].Begin(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);

        renderingInfo.flags = VK_RENDERING_SUSPENDING_BIT;
        renderingInfo.renderArea = { {}, windowSize };
        vkCmdBeginRendering(commandBuffer[1], &renderingInfo);
        vkCmdBindPipeline(commandBuffer[1], VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_triangle);
        vkCmdDraw(commandBuffer[1], 3, 1, 0, 0);
        vkCmdEndRendering(commandBuffer[1]);
    ...
    while (!glfwWindowShouldClose(pWindow)) {
        ...
        VkSubmitInfo submitInfo = {
            ...
            .pCommandBuffers = commandBuffer[0].Address(),
